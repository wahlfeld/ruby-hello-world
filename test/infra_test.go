...
var stateBucket string = fmt.Sprintf("%s-terratest-helloworld", uniqueID)
...
func TestTerraform(t *testing.T) {
	...
	test_structure.RunTestStage(t, "check_monitoring", func() {
		...
		validateResponse(t, monitoringURL, 15, 5*time.Second)
	})

	test_structure.RunTestStage(t, "check_helloworld", func() {
		terraformOptions := test_structure.LoadTerraformOptions(t, workingDir)

		helloWorldURL := terraform.Output(t, terraformOptions, "helloworld_url")

		validateResponse(t, helloWorldURL, 15, 5*time.Second)
	})
}
